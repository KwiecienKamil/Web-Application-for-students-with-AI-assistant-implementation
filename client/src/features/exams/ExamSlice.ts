...
export const completeExamAsync = createAsyncThunk<
	ExamData,
	{ accessToken: string; id: number; completed: boolean }
>("exams/completeExam", async ({ accessToken, id, completed }, thunkAPI) => {
	try {
		...
		return thunkAPI.rejectWithValue("Błąd complete exam");
	}
});
...
const examSlice = createSlice({
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(completeExamAsync.fulfilled, (state, action) => {
				const index = state.exams.findIndex((e) => e.id === action.payload.id);
